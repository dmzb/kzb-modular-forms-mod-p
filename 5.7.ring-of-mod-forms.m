// This code verifies some claims made in Example 5.7: the ring of modular forms of level 13 in char 0, char 2 and char 3
...
// Log canonical ring of X_0(13) in tame characteristics (e.g. char p = 5)
p := 5;
g := 0; // genus of X is 0
delta := 2; // X has 2 cusps
B := 3*p; // degree bound from KZB
F := GF(p); // may need to search over extensions of GF(p)
sig := [1/2, 1/2, 2/3, 2/3]; // signature of X as a stacky curve, from stacky structure code above
...
// Log canonical ring of X_0(13) in characteristic 2
p := 2;
g := 0; // genus of X is 0
delta := 2; // X has 2 cusps
...
B := 3*p; // degree bound from KZB
F := GF(p); // may need to search over extensions of GF(p)
sig := [(p-1)*(m+1)/p, 2/3, 2/3]; // refined signature of stacky curve
...
// Log canonical ring of X_0(13) in characteristic 3
p := 3;
g := 0; // genus of X is 0
delta := 2; // X has 2 cusps
m := 1; // ramification jump at wild pts is 1
B := 3*p; // degree bound from KZB
F := GF(p); // may need to search over extensions of GF(p)
sig := [1/2, 1/2, (p-1)*(m+1)/p]; // refined signature of stacky curve
